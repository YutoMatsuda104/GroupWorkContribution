...
io.on("connection", (socket) => {
  ...
  socket.on("step2_sendMyAvatarUpdate", (data) => {
    ...
    const [userColor, _userName, userPosition, userQuaternion, gazeX, gazeY] = data;
    ...
    const motionLog = {
      ...
      gazeX: gazeX !== undefined ? gazeX : "", // undefinedの場合は空文字列、それ以外は値を入れる
      gazeY: gazeY !== undefined ? gazeY : ""
    };
...
function saveMotionLogCSV(roomId, socketId, logs) {
  ...
  // ヘッダ行
  const header = "time,socketId,userName,posX,posY,posZ,rotX,rotY,rotZ,rotW,gazeX,gazeY\n";
